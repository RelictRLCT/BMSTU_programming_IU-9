...
;Соавтор Никитин Александр(наглая ложь)
    ...
    (cond
      ...
      ((and (not (null? slovar))
       ...
       (begin (set! возвраты (cons (list (+ 1 index)) возвраты))
              (interpretator
               (статья-есть-а-если-найду? slovar (vector-ref program index))
               stack slovar возвраты)))
      ((number? (vector-ref program index))
       (interpretator (+ index 1) (cons (vector-ref program index) stack) slovar возвраты))
      ...
      ((equal? (vector-ref program index) 'and)
       (begin (if (and (not (equal? (car stack) 0)) (not (equal? (cadr stack) 0)))                                                         
                  (begin (set! stack (cdr stack))
                         (set-car! stack -1))
                  (begin (set! stack (cdr stack))
                         (set-car! stack 0)))
      ...
      ((equal? (vector-ref program index) 'or)
       (begin (if (or (not (equal? (car stack) 0)) (not (equal? (cadr stack) 0)))                                                         
                  (begin (set! stack (cdr stack))
                         (set-car! stack -1))
                  (begin (set! stack (cdr stack))
                         (set-car! stack 0)))
      ...
      ((equal? (vector-ref program index) 'define)
       (interpretator (let loop ((index index)
                        ...
                        (if (equal? (vector-ref program index) 'end)
                            (+ index 1)
                            (loop (+ index 1) stack)))
                      stack (cons (list (vector-ref program (+ 1 index)) (+ 2 index))
                                  slovar)
                      возвраты))
      ...
      ((equal? (vector-ref program index) 'endif)
       (interpretator (+ 1 index) stack slovar возвраты)))))
